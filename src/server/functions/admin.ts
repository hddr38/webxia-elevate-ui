...
export const getAdminStats = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const { data: articlesAgg, error: articlesError } = await getSupabaseAdmin()
  ...
  const { data: realisationsAgg, error: realisationsError } = await getSupabaseAdmin()
    .rpc("get_realisations_stats");

  if (realisationsError) throw new Error(realisationsError.message);

  // AI Memory stats - single query
  const { data: memoryAgg, error: memoryError } = await getSupabaseAdmin()
